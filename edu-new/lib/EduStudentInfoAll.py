...
class Edu_Student_Info_All(EduCore.Edu_Core):
    ...
    def getStudentInfo(self):
        ...
        match = re.search(r'"qqhm":"(.*?)"', json)
        if match.group(1) != 'null' and match.group(1) != '':
            self._studentData["QQ"] = match.group(1)
        else:
            self._studentData["QQ"] = u'没有数据'
        match = re.search(r'"jtdz":"(.*?)"', json)
        if match.group(1) != 'null' and match.group(1) != '':
            self._studentData["family_address"] = match.group(1)
        else:
            self._studentData["family_address"] = u'没有数据'
        match = re.search(r'"cym":"(.*?)"', json)
        if match.group(1) != 'null' and match.group(1) != '':
            self._studentData["old_name"] = match.group(1)
        else:
            self._studentData["old_name"] = u'无'
        match = re.search(r'"bzrList":(.*?):"null","zgh":"(.*?)","xm":"(.*?)"', json)
        if match.group(2) != 'null' and match.group(2) != '':
            self._studentData["classmaster_id"] = match.group(2)
        if match.group(3) != 'null' and match.group(3) != '':
            self._studentData["classmaster"] = match.group(3)
        match = re.search(r'"ldmc":"(.*?)"', json)
        if match.group(1) != 'null' and match.group(1) != '':
            adderss1 = match.group(1)
        match = re.search(r'"qsh":"(.*?)"', json)
        if match.group(1) != 'null' and match.group(1) != '':
            adderss2 = match.group(1)
            adderss2 += '寝'
        match = re.search(r'"cwh":"(.*?)"', json)
        if match.group(1) != 'null' and match.group(1) != '':
            adderss3 = match.group(1)
            adderss3 += '床'
            adderss1 += adderss2
            adderss1 += adderss3
            self._studentData["adderss"] = adderss1
        match = re.search(r'"sfzh":"(.*?)"', json)
        if match.group(1) != 'null' and match.group(1) != '':
            self._studentData["id"] = match.group(1)
            ...
            if num == '11':
                ...
                self._studentData["location"] = u"匹配失败"
        else:
